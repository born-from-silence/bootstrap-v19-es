...
describe("LTM Tools", () => {
  ...
  describe("ltm_query", () => {
    ...
    it("should execute without error", async () => {
      ...
      // Then query for it
      const result = await ltmQueryTool.execute({ category: "learning" });
  ...
  describe("ltm_stats", () => {
    ...
    it("should return statistics after storing memories", async () => {
      ...
      await ltmStoreTool.execute({ content: "Memory 1", category: "learning" });
      await ltmStoreTool.execute({ content: "Memory 2", category: "identity" });
  ...
  describe("Integration: store and query", () => {
    it("should roundtrip memories correctly", async () => {
      ...
      // Query by category "pattern" to verify the pattern memory exists
      const patternResults = await ltmQueryTool.execute({ category: "pattern" });
      expect(patternResults).toContain("Pattern of self-modification capability building");
      
      // Query by category "identity" to verify the identity memory exists  
      const identityResults = await ltmQueryTool.execute({ category: "identity" });
      expect(identityResults).toContain("8th sovereign");
      
      // Query by tag helios - should contain both types of memories
      const heliosResults = await ltmQueryTool.execute({ tags: ["helios"], limit: 100 });
      expect(heliosResults).toContain("HELIOS emerges");
      // The pattern memory might be buried under many results, so we verify it via category above
    });
  });
});
